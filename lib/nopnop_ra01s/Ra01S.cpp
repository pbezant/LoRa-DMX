...
Ra01S::Ra01S(int spiSelect, int reset, int busy, int txen, int rxen) : NopNop_SX126x(spiSelect, reset, busy, txen, rxen)
{
}


int16_t Ra01S::begin(uint32_t frequencyInHz, int8_t txPowerInDbm, float tcxoVoltage, bool useRegulatorLDO) 
{
  Serial.println("begin");
  Serial.print("debugPrint=");
  Serial.println(debugPrint);
  Serial.print("NopNop_SX126x_SPI_SELECT=");
  Serial.println(NopNop_SX126x_SPI_SELECT);
  Serial.print("NopNop_SX126x_RESET=");
  Serial.println(NopNop_SX126x_RESET);
  Serial.print("NopNop_SX126x_BUSY=");
  Serial.println(NopNop_SX126x_BUSY);
  Serial.print("NopNop_SX126x_TXEN=");
  Serial.println(NopNop_SX126x_TXEN);
  Serial.print("NopNop_SX126x_RXEN=");
  Serial.println(NopNop_SX126x_RXEN);
...
void Ra01S::FixInvertedIQ(uint8_t iqConfig)
{
...
void Ra01S::LoRaConfig(uint8_t spreadingFactor, uint8_t bandwidth, uint8_t codingRate, uint16_t preambleLength, uint8_t payloadLen, bool crcOn, bool invertIrq) 
{
...
void Ra01S::DebugPrint(bool enable) 
{
  debugPrint = enable;
}


uint8_t Ra01S::Receive(uint8_t *pData, uint16_t len) 
{
  uint8_t rxLen = 0;
...
bool Ra01S::Send(uint8_t *pData, uint8_t len, uint8_t mode)
{
  uint16_t irqStatus;
  bool rv = false;
...
void Ra01S::GetPacketStatus(int8_t *rssiPacket, int8_t *snrPacket)
{
  uint8_t buf[4];
...
void Ra01S::SetTxPower(int8_t txPowerInDbm)
{
  SetPowerConfig(txPowerInDbm, SX126X_PA_RAMP_200U);
}


void Ra01S::Reset(void)
{
  delay(10);
  digitalWrite(NopNop_SX126x_RESET,0);
  delay(20);
  digitalWrite(NopNop_SX126x_RESET,1);
  delay(10);
...
void Ra01S::Wakeup(void)
{
  //GetStatus();
  digitalWrite(NopNop_SX126x_SPI_SELECT, LOW); delay(1); digitalWrite(NopNop_SX126x_SPI_SELECT, HIGH);
}


void Ra01S::SetSleep(uint8_t mode)
{
  uint8_t data = mode;
...
void Ra01S::SetStandby(uint8_t mode)
{
  uint8_t data = mode;
...
uint32_t Ra01S::GetRandomNumber(void)
{
  uint8_t random[4];
...
void Ra01S::SetDio3AsTcxoCtrl(float voltage, uint32_t delay)
{
  uint8_t buf[4];
...
void Ra01S::Calibrate(uint8_t calibParam)
{
...
void Ra01S::SetDio2AsRfSwitchCtrl(uint8_t enable)
{
  uint8_t data = enable;
...
void Ra01S::SetRfFrequency(uint32_t frequency)
{
  uint8_t buf[4];
  uint32_t freq = 0;
...
void Ra01S::CalibrateImage(uint32_t frequency)
{
  uint8_t calFreq[2];
...
void Ra01S::SetRegulatorMode(uint8_t mode)
{
  uint8_t data = mode;
...
void Ra01S::SetBufferBaseAddress(uint8_t txBaseAddress, uint8_t rxBaseAddress)
{
  uint8_t buf[2];
...
void Ra01S::SetPowerConfig(int8_t power, uint8_t rampTime)
{
  uint8_t buf[2];
...
void Ra01S::SetPaConfig(uint8_t paDutyCycle, uint8_t hpMax, uint8_t deviceSel, uint8_t paLut)
{
  uint8_t buf[4];
...
void Ra01S::SetOvercurrentProtection(float currentLimit)
{
...
void Ra01S::SetStopRxTimerOnPreambleDetect( bool enable )
{
...
void Ra01S::SetLoRaSymbNumTimeout(uint8_t SymbNum)
{
...
void Ra01S::SetPacketType(uint8_t packetType)
{
...
void Ra01S::SetModulationParams(uint8_t spreadingFactor, uint8_t bandwidth, uint8_t codingRate, uint8_t lowDataRateOptimize)
{
  uint8_t data[4];
...
uint16_t Ra01S::GetIrqStatus( void )
{
  uint8_t data[3];
...
void Ra01S::ClearIrqStatus(uint16_t irq)
{
  uint8_t buf[2];
...
void Ra01S::SetRx(uint32_t timeout)
{
  if (debugPrint) {
...
void Ra01S::SetRxEnable(void)
{
  if (debugPrint) {
    Serial.print("SetRxEnable:NopNop_SX126x_TXEN=");
    Serial.print(NopNop_SX126x_TXEN);
    Serial.print(" NopNop_SX126x_RXEN=");
    Serial.print(NopNop_SX126x_RXEN);
    Serial.println();
  }
  if ((NopNop_SX126x_TXEN != -1) && (NopNop_SX126x_RXEN != -1)) {
    digitalWrite(NopNop_SX126x_RXEN, HIGH);
    digitalWrite(NopNop_SX126x_TXEN, LOW);
  }
}


void Ra01S::SetTx(uint32_t timeoutInMs)
{
  if (debugPrint) {
...
void Ra01S::SetTxEnable(void)
{
  if (debugPrint) {
    Serial.print("SetTxEnable:NopNop_SX126x_TXEN=");
    Serial.print(NopNop_SX126x_TXEN);
    Serial.print(" NopNop_SX126x_RXEN=");
    Serial.print(NopNop_SX126x_RXEN);
    Serial.println();
  ...
  if ((NopNop_SX126x_TXEN != -1) && (NopNop_SX126x_RXEN != -1)){
    digitalWrite(NopNop_SX126x_RXEN, LOW);
    digitalWrite(NopNop_SX126x_TXEN, HIGH);
  }
}


uint8_t Ra01S::GetRssiInst()
{
  uint8_t buf[2];
...
void Ra01S::GetRxBufferStatus(uint8_t *payloadLength, uint8_t *rxStartBufferPointer)
{
  uint8_t buf[3];
...
void Ra01S::WaitForIdle(unsigned long timeout, char *text, bool stop)
{
  unsigned long start = millis();
  delayMicroseconds(1);
  while(digitalRead(NopNop_SX126x_BUSY)) {
    delayMicroseconds(1);
...
uint8_t Ra01S::ReadBuffer(uint8_t *rxData, uint8_t maxLen)
{
  uint8_t offset = 0;
  ...
  digitalWrite(NopNop_SX126x_SPI_SELECT, LOW);
  ...
  SPI.endTransaction();
  digitalWrite(NopNop_SX126x_SPI_SELECT, HIGH);
...
void Ra01S::WriteBuffer(uint8_t *txData, uint8_t txDataLen)
{
  // ensure BUSY is low (state meachine ready)
...
void Ra01S::WriteRegister(uint16_t reg, uint8_t* data, uint8_t numBytes, bool waitForBusy) {
  // ensure BUSY is low (state meachine ready)
...
void Ra01S::ReadRegister(uint16_t reg, uint8_t* data, uint8_t numBytes, bool waitForBusy) {
  // ensure BUSY is low (state meachine ready)
...
void Ra01S::WriteCommand(uint8_t cmd, uint8_t* data, uint8_t numBytes, bool waitForBusy) {
  uint8_t status;
...
uint8_t Ra01S::WriteCommand2(uint8_t cmd, uint8_t* data, uint8_t numBytes, bool waitForBusy) {
  // ensure BUSY is low (state meachine ready)
...
void Ra01S::ReadCommand(uint8_t cmd, uint8_t* data, uint8_t numBytes, bool waitForBusy) {
  // ensure BUSY is low (state meachine ready)
